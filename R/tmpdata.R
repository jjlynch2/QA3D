write.tmp.data <- function(A, B, comp_name) {
	mainDir1 <- paste(tempdir(), "/1/", sep="")
	mainDir2 <- paste(tempdir(), "/2/", sep="")
...
import.tmp.data <- function(comp_name) {
	file1 <- paste(tempdir(), "/1/", comp_name, sep="")
	file2 <- paste(tempdir(), "/2/", comp_name, sep="")
	data1 <- read.csv(file1, stringsAsFactors=FALSE)
	data2 <- read.csv(file2, stringsAsFactors=FALSE)
	return(list(data1, data2))
}

delete.tmp.data <- function() {
	file1 <- paste(tempdir(), "/1/", sep="")
	file2 <- paste(tempdir(), "/2/", sep="")
